...
object UnifiedExceptionHandler {
    ...
    suspend fun <T> handleSuspendWithGithubData(function: suspend () -> T): com.lemon.mcdevmanagermp.utils.NetworkState<T> {
        return try {
            when (val result = function.invoke()) {
                is LatestReleaseBean -> {
                    val uniData = _root_ide_package_.com.lemon.mcdevmanagermp.utils.ResponseData(
                        "200",
                        result
                    )
                    parseData(uniData)
                ...
                else -> com.lemon.mcdevmanagermp.utils.NetworkState.Error("函数调用错误，非Github更新接口请使用handleSuspend")
            }
        } catch (e: SocketTimeoutException) {
            Logger.e("$TAG:链接超时\n$e")
            return com.lemon.mcdevmanagermp.utils.NetworkState.Error(
                "网络好像被末影人搬走了",
                SocketTimeoutException("网络好像被末影人搬走了")
            )
        } catch (e: ConnectException) {
            ...
            if (e.code() == 403) {
                ...
                return com.lemon.mcdevmanagermp.utils.NetworkState.Error("请求过于频繁,请稍后再次尝试", e)
            } else return com.lemon.mcdevmanagermp.utils.NetworkState.Error("未知错误，请联系管理员", e)
        } catch (e: Exception) {
            e.message?.let { Logger.e("$TAG:$it") } ?: Logger.e(e::class.toString())
            return com.lemon.mcdevmanagermp.utils.NetworkState.Error("未知错误，请联系管理员", e)
        }
    }

    suspend fun <T> handleSuspendWithNeteaseData(function: suspend () -> T): com.lemon.mcdevmanagermp.utils.NetworkState<String> {
        return try {
            when (val result = function.invoke()) {
                is TicketBean -> {
                    val uniData = _root_ide_package_.com.lemon.mcdevmanagermp.utils.ResponseData(
                ...
                is PowerBean -> {
                    val uniData =
                        _root_ide_package_.com.lemon.mcdevmanagermp.utils.ResponseData(
                            result.ret.toString(),
                            dataJsonToString(result.pVInfo)
                        )
                    parseData(uniData, noNeedRefreshCookies = true)
                }
                ...
                else -> com.lemon.mcdevmanagermp.utils.NetworkState.Error("函数调用错误，非网易登录接口请使用handleSuspend")
            }
        } catch (e: SocketTimeoutException) {
            Logger.e("$TAG:链接超时\n$e")
            return com.lemon.mcdevmanagermp.utils.NetworkState.Error(
            ...
            if (e.code() == 401) {
                Logger.e("$TAG:Token失效\n$e")
                return com.lemon.mcdevmanagermp.utils.NetworkState.Error("登录过期啦!", LoginException("登录过期啦!"))
            } else return com.lemon.mcdevmanagermp.utils.NetworkState.Error("未知错误，请联系管理员", e)
        } catch (e: Exception) {
            e.message?.let { Logger.e("$TAG:$it") } ?: Logger.e(e::class.toString())
            return com.lemon.mcdevmanagermp.utils.NetworkState.Error("未知错误，请联系管理员", e)
    ...
    private fun <T> parseData(
        result: com.lemon.mcdevmanagermp.utils.ResponseData<T>,
        noNeedRefreshCookies: Boolean = false
    ): com.lemon.mcdevmanagermp.utils.NetworkState<T> {
        ...
        return when (result.status) {
            "200", "ok", "OK", "Ok" -> result.data?.let { com.lemon.mcdevmanagermp.utils.NetworkState.Success(result.data) }
                ?: com.lemon.mcdevmanagermp.utils.NetworkState.Success(msg = result.msg ?: result.status)

            "201" -> result.data?.let { com.lemon.mcdevmanagermp.utils.NetworkState.Success(result.data) }
                ?: com.lemon.mcdevmanagermp.utils.NetworkState.Success(msg = result.msg ?: result.status)

            "401", "no_login" -> com.lemon.mcdevmanagermp.utils.NetworkState.Error(
                "登录过期了，请重新登录",
                com.lemon.mcdevmanagermp.utils.CookiesExpiredException
            )

            else -> com.lemon.mcdevmanagermp.utils.NetworkState.Error(result.msg ?: result.status)
        }
    }
